...
reg [2:0] rst_sync_sdram_c /* synthesis syn_keep=1 */ /* synthesis syn_dont_touch=1 */ /* synthesis syn_preserve=1 */;
...
always @(posedge clk_85m or negedge pll_lock) begin
  if (!pll_lock)
    rst_sync_sdram_c <= 3'b000;
  else
    rst_sync_sdram_c <= {rst_sync_sdram_c[1:0], 1'b1};
end
...
wire rst_sdram_c   = ~rst_sync_sdram_c[2];
wire rst_sdram_c_n = rst_sync_sdram_c[2];
...
sdram #(
	...
	.reset_n    ( rst_sdram_c_n ), // init signal after FPGA config to initialize RAM
